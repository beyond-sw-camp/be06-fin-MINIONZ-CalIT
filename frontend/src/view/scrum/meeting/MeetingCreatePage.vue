...
const addMeeting = () => {
  const data ={
      ...
      participants: friendsStore.selectedParticipants.map(participant => participant.searchUserIdx)
  };
  console.log(data);
    ...
    <div class="meeting-wrap">
        ...
        <span class="column">
          ...
          라벨 추가하기
                    ...
                    <button
                            v-for="(label, index) in sprintLabelStore.labels"
                            :key="index"
                            class="label-button"
                            :style="`background-color:${label.color.backgroundColor}`"
                    >
                        {{ label.labelName }}
                    </button>
...
.save-button {
    ...
    border-radius: 5px;
    cursor: pointer;
//margin-top: 20px; width: 150px; //margin-left: auto;
}
