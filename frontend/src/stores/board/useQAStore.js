...
export const useQAStore = defineStore('qaStore', () => {
  ...
  const getPostDetail = async (boardId) => {
    try {
      const response = await axiosInstance.get(
        `/api/qaboard/search?boardId=${boardId}`
      );
  ...
  const getPostList = async (workspaceId, page, size) => {
    try {
      page = page - 1;
      const response = await axiosInstance.get(
        `/api/qaboard/search-all?workspaceId=${workspaceId}&page=${page}&size=${size}`
      );
  ...
  const deletePost = async (id) => {
    try {
      const response = await axiosInstance.delete('/api/qaboard/delete', {
        data: { id },
      });
      return response.data.result;
    } catch (error) {
