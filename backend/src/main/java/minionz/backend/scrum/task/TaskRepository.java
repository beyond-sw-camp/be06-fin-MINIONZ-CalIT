...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    @Query("SELECT COUNT(*) AS mydonetask_count FROM Task t " +
            "JOIN FETCH TaskParticipation tp ON t = tp.task " +
            "JOIN FETCH Sprint s ON t.sprint = s " +
            "WHERE tp.user.userId = :userId AND s.endDate > CURRENT_TIMESTAMP AND t.status = minionz.backend.scrum.task.model.TaskStatus.DONE ")
    ...
    @Query("SELECT t FROM Task t " +
            ...
            "CASE t.priority " +
            "WHEN minionz.backend.scrum.task.model.TaskLevel.HIGH THEN 1 " +
            "WHEN minionz.backend.scrum.task.model.TaskLevel.MED THEN 2 " +
            "WHEN minionz.backend.scrum.task.model.TaskLevel.LOW THEN 3 " +
            "END ASC")
